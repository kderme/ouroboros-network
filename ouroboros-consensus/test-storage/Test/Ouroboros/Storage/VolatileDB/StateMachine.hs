...
data Success
    ...
    | Blocks   [BlockId]
    | Bl       Bool
    | IsMember [Bool] -- ^ We compare two functions based on their results on
                      -- a list of inputs.
...
runPure dbm (CmdErr cmd err) =
  ...
  where
    tnc :: EH.ThrowCantCatch (VolatileDBError BlockId) PureM
    tnc = EH.throwCantCatch EH.exceptT

    go :: PureM Success
    go = case cmd of
      GetBlock bid       -> Blob <$> getBlockModel tnc bid
      GetBlockIds        -> Blocks <$> getBlockIdsModel tnc
      PutBlock b pb      -> Unit <$> putBlockModel tnc err
      ...
      DuplicateBlock file bid _pbid -> withClosedDB $
...
runDB db cmd env@Internal.VolatileDBEnv{..} = case cmd of
    ...
    CreateFile -> do
      Internal.withState env $ createFile
      withClosedDB $ return ()
    CreateInvalidFile ->
      withClosedDB $
        withFile _dbHasFS (mkFsPath ["invalidFileName.dat"]) (AppendMode MustBeNew)
          $ \_hndl -> return ()
    DuplicateBlock _file  bid preBid -> do
      let specialEnc = toBinary (bid, preBid)
      _ <- Internal.withState env $ \hasFS st ->
        hPut hasFS (_handle $ Internal._currentWriteHandle st)
...
prop_sequential =
    forAllCommands smUnused Nothing $ \cmds -> monadicIO $ do
        ...
        prettyCommands smUnused hist
            ...
            $ tabulate "IsMember: Total number of True's" [groupIsMember $ isMemberTrue events]
