...
-- | Tries to corrupt the db by creating the next file.
-- The db should be able to succesfully recover.
createFile :: forall h m blockId. IOLike m
           => HasFS m h
           -> InternalState blockId m h
           -> m ()
createFile hasFS st =
    withFile hasFS path (AppendMode MustBeNew) $ \_hndl ->
      return ()
  where
    nextFd = Internal._nextNewFileId st
    path = Internal.filePath nextFd
