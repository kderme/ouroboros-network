...
generateCmd Model {..} = At <$> frequency
    ...
    , (3, do
            ...
            slotNo  <- frequency
              ...
              , (if maybe False testBlockIsEBB mbPrevBlock then 7 else 0,
                 return lastSlot)
  ...
  where
    ...
    genBlockInThePast
      | noBlocks  = discard
      | otherwise = elements $ map testBlockFromLazyByteString
                  $ catMaybes dbmChain

    genEBBInThePast :: Gen TestBlock
    genEBBInThePast
      | noEBBs    = discard
      | otherwise = elements $ map (testBlockFromLazyByteString . snd)
                  $ Map.elems dbmEBBs
...
prop_sequential = forAllCommands smUnused Nothing $ \cmds -> QC.monadicIO $ do
  ...
  where
    ...
    isEBB b | testBlockIsEBB b = Just (blockHash b)
            | otherwise        = Nothing
